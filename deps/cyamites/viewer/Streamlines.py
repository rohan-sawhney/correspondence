...
def prepareVectorField(mesh, vectorFieldName, definedOn, isTangentVector=False, nSym=1):
    ...
    if definedOn == 'vertex':
        ...
        if isTangentVector:
            ...
            for face in mesh.faces:
                # angle = cmath.phase(getattr(face, vectorFieldName)**(1.0/nSym))
                angle = getattr(face, vectorFieldName)
                vecR3 = face.tangentAngleInR3(angle)
                face._streamVec = normalized(face.projectToTangentSpace(vecR3))
    ...
    for face in mesh.faces:
        ...
        xDir = face.anyHalfEdge.vec
        yDir = cross(xDir, face.normal)
        v0 = face.anyHalfEdge.vertex.pos
...
def traceStreamlineThroughFace(startFace, startPoint, currV, valueCache):
    ...
    newPoint = acrossHe.vertex.pos - t * acrossHe.vec + (uMin * 0.00001) * uVecR3
