...
# Cyamites imports
from ..core.HalfEdgeMesh import HalfEdgeMesh, Vertex, Edge, Face
from ..core.Utilities import normalized, norm, cross
...
class MeshDisplay(object):
    ...
    def __init__(self, windowTitle='Cyamites MeshDisplay', mesh=None,
    ...
    def generateFaceData(self, pick=False):
        ...
        for (i, face) in enumerate(self.mesh.faces):
            ...
            facePos[3*i  ,:] = v1.pos
            facePos[3*i+1,:] = v2.pos
            facePos[3*i+2,:] = v3.pos
        ...
        if not useCache or self.vectorAttrName not in self.vectorArrowCache:
            ...
            if self.vectorIsTangent:
                ...
                if self.vectorDefinedAt == 'vertex':
                    ...
                    vecStart = np.zeros((nTotalVector,3))
                    vecEnd = np.zeros((nTotalVector,3))
                    ...
                    for (iVert, vert) in enumerate(self.mesh.verts):
                        for iRot in range(nSym):
                            ...
                            vecDir = np.array(refDir.rotate_around(eu.Vector3(*vert.normal), theta + rotInc * iRot))
                            vecEnd[(nSym*iVert + iRot),:] = vert.pos + normalized(vecDir) * unitVectorLength
    
                elif self.vectorDefinedAt == 'face':

                    nTotalVector = self.nFaces * nSym
                    vecStart = np.zeros((nTotalVector,3))
                    vecEnd = np.zeros((nTotalVector,3))
                    ...
                    for (iFace, face) in enumerate(self.mesh.faces):
                        for iRot in range(nSym):
                            ...
                            # The base point
                            vecStart[(nSym*iFace + iRot),:] = face.center

                            # The draw-to point
                            theta = getattr(face, self.vectorAttrName)
                            refDir = eu.Vector3(*getattr(face, self.vectorRefDirAttrName))
                            vecDir = np.array(refDir.rotate_around(eu.Vector3(*face.normal), theta + rotInc * iRot))
                            vecEnd[(nSym*iFace + iRot),:] = face.center + normalized(vecDir) * unitVectorLength

                else:
                    raise ValueError("Uh oh. Unrecognize vectorDefinedAt value")
            ...
            # Generate vectors in R3
                ...
                if self.vectorDefinedAt == 'vertex':
                    ...
                    for (iVert, vert) in enumerate(self.mesh.verts):
                        vecStart[iVert,:] = vert.pos
                        vecEnd[iVert,:] = vert.pos + getattr(vert, self.vectorAttrName)
            ...
            # (the previous computations need to be performed at double precision to make sure nothing gets eaten)
            vecStart = vecStart.astype(np.float32)
            vecEnd = vecEnd.astype(np.float32)
    ...
    def generateStreamlines(self, useCache=True, colormap='Spectral'):
        ...
        if not useCache or self.vectorAttrName not in self.streamlineCache:
            ...
            # Parameters
            nLines = int(round(len(self.mesh.faces) * self.vectorNsym / 5.0 * self.nStreamlinesCoef))
    ...
    def computeScale(self):
        ...
        for v in self.mesh.verts:
            for i in range(3):
                bboxMax[i] = max((bboxMax[i], v.pos[i]))
                bboxMin[i] = min((bboxMin[i], v.pos[i]))
        ...
        lengths = sorted([norm(e.anyHalfEdge.vec) for e in self.mesh.edges])
    ...
    def redraw(self):
        ...
        if self.drawVertices:
            self.vertProg.draw()
        if self.streamlinesEnabled and self.drawStreamlines:
    ...
    def registerVectorToggleCallbacks(self, vectorList, enableStreamlines=True):
        ...
        if self.mesh is None:
            raise ValueError("ERROR: Cannot register a vector toggle callback until after the mesh has been set.")
