...
class HalfEdge(object):
    ...
    def oppAngle(self):
        ...
        A = -normalized(self.next.vec)
        B = normalized(self.next.next.vec)
    ...
    def edgeAngle(self):
        ...
        while currOutEdge is not self:
            ...
            A = normalized(currOutEdge.vec)
            B = normalized(nextEdge.vec)
...
class Vertex(object):
    ...
    def halfedgeTo(self, otherVertex):
        """
        Returns the halfedge from this vertex to otherVertex, or
        None if there is no such halfedge.

        Currently O(degree)
        """
        for he in self.adjacentHalfEdges():
            if he.vertex is otherVertex:
                return he
        return None
    ...
    def angleDefect(self):
        ...
        if(self.isBoundary):
            ...
            angSum = 0.0
            vList = [normalized(h.vec) for h in self.adjacentHalfEdges()]
    ...
    def referenceDirectionR3(self):
        ...
        rInPlane = normalized(r - rNorm)
        referenceDirectionR3 = rInPlane

        return referenceDirectionR3

    def tangentAngleInR3(self, theta):
        """
        An angle defined in the tangent space (relative to the refernee edge)
        defines a unit vector in R3 at this vertex. Return that R3 vector.

        Should only be called after a reference edge has been selected.
        (mesh.assignReferenceDirections())
        """

        axis = eu.Vector3(*self.normal)
        baseVec = self.referenceDirectionR3
        # make sure the basevec lies in the tangent plane
        baseVec = normalized(baseVec - dot(baseVec, self.normal) * self.normal)
        baseVec = eu.Vector3(*baseVec)
        result = baseVec.rotate_around(axis, theta)
        return np.array(result)


    def tangentVectorToAngle(self, vec):
        """
        Given a vector in the tangent space, return the angle against the reference
        direction for that vector.
        """
       
        # Normalize and project to tangent space
        nVec = normalized(vec)
        nVec = normalized(nVec - dot(nVec, self.normal) * self.normal)

        # Compute the angle
        dotVal = clamp(dot(nVec, self.referenceDirectionR3) , -1.0, 1.0) 
        theta = acos(dotVal)
        if dot(cross(self.referenceDirectionR3, nVec), self.normal) < 0:
            theta = 2*pi - theta
        theta = regAngle(theta)
...
class Face(object):
    ...
    # Iterate over the (real) faces that are adjacent to this face
    def adjacentFaces(self, skipImaginary=True):
        ...
        while True:
            ...
            if curr.twin.face.isReal or (not skipImaginary):
                yield curr.twin.face
    ...
    def normal(self):
        ...
        n = normalized(cross(v[1].pos - v[0].pos, v[2].pos - v[0].pos))
    ...
    def tangentAngleInR3(self, theta):
        """
        An angle defined in the tangent space (relative to the reference direction)
        defines a unit vector in R3 at this vertex. Return that R3 vector.

        Should only be called after a reference dir has been selected.
        (mesh.assignReferenceDirections())
        """
        # TODO implement a reference direction creator for faces
